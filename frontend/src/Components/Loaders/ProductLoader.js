...
const ProductLoader = ({wrap}) => {
  return (
    <div className={`justify-center flex ${wrap} gap-4 min-[375px]:gap-6`}>
      {Array.from({ length: 12 }).map((element, index) => (
        <div key={index} className="w-44 md:w-48 lg:w-52 xl:w-56  bg-gray-200 animate-pulse rounded-lg">
          <div className="w-full h-44 md:h-48 lg:h-52 xl:h-56 bg-gray-300 rounded-t-lg"></div>
